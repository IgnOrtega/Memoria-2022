def compute_c_ab_sample_7_8(dict sample_aristas, set set_infectados):
    cdef int suma_num_c_ab = 0, suma_den_c_ab = 0
    cdef int suma_num_c_ba = 0, suma_den_c_ba = 0

    for v, u in sample_aristas.items():
        cdef int v_f_directriz = int(v in set_infectados)
        cdef int v_g_directriz = 1 - v_f_directriz

        cdef int u_f_directriz = int(u in set_infectados)
        cdef int u_g_directriz = 1 - u_f_directriz
